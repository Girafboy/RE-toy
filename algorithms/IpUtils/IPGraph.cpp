...
namespace ip {
    ...
    IPGraph::IPGraph(int size) {
        vsize = size;
        vl = VertexList(size);
        graph = GRA(size, In_OutList());
    }

    IPGraph::IPGraph(GRA& g, VertexList& vlist) {
        vsize = vlist.size();
        graph = g;
        vl = vlist;
    }

    IPGraph::IPGraph(istream& in) {
        readGraph(in);
    }

    IPGraph::~IPGraph() {}

    void IPGraph::printGraph(int i, ostream& out) {
        writeGraph(out,i);
    }

    void IPGraph::printGraph(int i) {
        writeGraph(cout,i);
    }
    ...
    void IPGraph::strTrimRight(string& str) {
        string whitespaces(" \t\r\n");
    ...
    void IPGraph::readGraph(istream& in) {
        string buf;
        getline(in, buf);
...
//		istringstream(buf) >> sid;
    ...
    void IPGraph::readGraph2(istream& in) {
        string line;
        int u, v;
        vector<string> edge;
...
//		vl[i].id=i;
    ...
    void IPGraph::readGraph3(istream& in) {
        string buf;
        int n;
...
//              istringstream(buf) >> sid;
    ...
    void IPGraph::convertGraph(const Graph &input_graph) {
        vsize = input_graph.size();
        vl = VertexList(vsize, Vertex());
        graph = GRA(vsize, In_OutList());
        for (int sid = 0; sid < vsize; ++sid) {
            vl[sid].outdegree = input_graph.getOutDegree(sid);
            vl[sid].indegree = input_graph.getInDegree(sid);
            graph[sid].outList.reserve(vl[sid].outdegree);
            for (int tid : input_graph.getSuccessors(sid)) {
                addEdge(sid,tid);
            }
        }
    }
...
/*
void IPGraph::readGraphFromInlist(istream& in) {
	string buf;
	getline(in, buf);
...
*/
    ...
    void IPGraph::writeGraph(ostream& out, int j) {
    ...
    void IPGraph::writeGraph(ostream& out) {
        cout << "IPGraph size = " << graph.size() << endl;
        out << "graph_for_greach" << endl;
        out << vl.size() << endl;
    ...
    vector<string>& IPGraph::split(const string &s, char delim, vector<string> &elems) {
        int begin, end;
    ...
    vector<string> IPGraph::split(const string &s, char delim) {
        vector<string> elems;
        return split(s, delim, elems);
    }

    void IPGraph::sortEdges() {
        GRA::iterator git;
    ...
    void IPGraph::addVertex(int vid) {
        if (vid >= vl.size()) {
            int size = vl.size();
    ...
    void IPGraph::addEdge(int sid, int tid) {
        if (sid >= vl.size())
            addVertex(sid);
        if (tid >= vl.size())
            addVertex(tid);
...
// check whether the edge from src to trg is in the graph
    bool IPGraph::hasEdge(int src, int trg) {
...
// return vertex list of graph
    VertexList& IPGraph::vertices() {
        return vl;
    }

    IPGraph::IPGraph(const IPGraph& g) {
        if (this != &g) {
            this->graph = g.graph;
            this->vl = g.vl;
            this->vsize = g.vsize;
    ...
    IPGraph& IPGraph::operator=(const IPGraph& g) {
        if (this != &g) {
            graph = g.graph;
            vl = g.vl;
            vsize = g.vsize;
    ...
    string IPGraph::getFilename(){
        return name;
    }
    void IPGraph::setFilename(string name){
        this->name.swap(name);
    }
// get a specified vertex property
    Vertex& IPGraph::operator[](const int vid) {
        return vl[vid];
    }


    void IPGraph::RandomDAGGenerator(int n, int m, const string& filename){
        int i=0;
        int u, v;
    ...
    void IPGraph::ComputeLayerLabel(){
        int i, k;
        int u,v;
        vector<int> indegreeRecord(vsize);
        uint16_t layer=1;
    ...
    void IPGraph::reverseLayerSeparation(){
...
//	output.close();
    ...
    void IPGraph::countIndegree() const {
    ...
    void IPGraph::checkLayerSeparation(){
        int u,v;
        bool flag=false;
    ...
    void IPGraph::checkReverseLayerSeparation(){
        int u,v;
        bool flag=false;
    ...
    void IPGraph::BFS(const int root){
        queue<int> que;
    ...
    int IPGraph::FindStructureEq(vector<bool>& SEflag){
        SEflag.clear();
    ...
    void IPGraph::PrintMapGraph(const string& filename){
        ofstream output(filename.c_str());
    ...
    void IPGraph::PrintReOrderedGraph(const vector<int>& order){
    ...
    void IPGraph::PrintReOrderedGraph2(const vector<int>& order){
...
//		sort(ReOrderedGraph[u].begin(), ReOrderedGraph[u].end(),EdgeSort(u));
    ...
    unsigned int IPGraph::BFSQuery(const unsigned int u, const unsigned int v){
        if(u==v)
            return 0;
    ...
    unsigned int IPGraph::BFSSubQuery(set<unsigned int>& qvertice, const unsigned int v, unsigned int depth, vector<bool>& flag){
    ...
    void IPGraph::ComputeOutSet(const unsigned int u, set<unsigned int>& out){
        unsigned int v;
    ...
    void IPGraph::ComputeInSet(const unsigned int v, set<unsigned int>& in){
        unsigned int u;
        pair< set<unsigned int>::iterator, bool > flag;
    ...
    void IPGraph::ComputeProLabelOut(){
        int u;
    ...
    void IPGraph::ComputeProLabelIn(){
        int v;
        unsigned int tmpsize;
    ...
    void IPGraph::VectorUnion(vector<int>& v1, const vector<int>& v2){
...
//              }
    ...
    void IPGraph::SubComputeProLabelOut(int u){
...
*/
    ...
    void IPGraph::SubComputeProLabelIn(int v){
...
*/
    ...
    void IPGraph::PrintIndex(const string& filename){
        ofstream output(filename.c_str());
    ...
    void IPGraph::ReadIndex(const string& filename){
    ...
    void IPGraph::ComputeHugeNodeList(){
    ...
    void IPGraph::HugeNodeListUnion(vector<int>& v1, const vector<int>& v2){
...
*/
    ...
    void IPGraph::IndexQueryTest(const string& indexfilename, const string& queryfilename){
        int u, v;
        bool ProLflag;
        clock_t start, end;
...
//			cout << "i: " << i << '\t' << (*qit).first << ' ' << (*qit).second << ' ' << reachflag[i] << endl;
    ...
    bool IPGraph::IPtest(const int u, const int v){
        int it1=0, it2=0;
    ...
    bool IPGraph::ProLabelQueryDFS2(const int u, const int v){
//      int tmpu;
...
//              if(flag[*it]!=querycnt){
    ...
    bool IPGraph::IsContradict(vector<int>& v1, vector<int>& v2){
