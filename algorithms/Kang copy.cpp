...
void Kang::merge(std::vector<std::vector<int>*> &jumps, std::vector<int> &merged_jump) {
    ...
    static auto cmp = [](const std::vector<int>* left, const std::vector<int>* right) {return left->size()>right->size();};
    std::priority_queue<std::vector<int>*, std::vector<std::vector<int>*>, decltype(cmp)> heap(jumps.begin(), jumps.end(), cmp);
...
// O(kdf + f log f)
void Kang::shrink(std::vector<int> &fakejump, size_t limitation) {
    ...
    if (indices.size() > limitation - 1) {
        std::nth_element(indices.begin(), indices.begin() + limitation - 1, indices.end(), std::greater<void>());
    }
    std::vector<int> slice;
    for (int i = 0; i < std::min(indices.size(), limitation - 1); ++i) {
...
void Kang::DFS(int u) {
    ...
    for (const int v : topo_adj_list[u])
        if (nodes[v].shallowjump->size() + nodes[v].deepjump->size() >= k) {
    ...
    if (nodes[u].shallowjump->size() + nodes[u].deepjump->size() >= k) {
...
Kang::Kang(int k, size_t f) : k(k), f(f) {}
...
std::string Kang::getParams() const {
    return "k=" + std::to_string(k) + " " + "f=" + std::to_string(f);
}

long long Kang::getIndexSize() const {
    long long index_size = nodes.size() * (2 * sizeof(int));
    for (const auto &node : nodes) {
        index_size += ((node.shallowjump?node.shallowjump->size():0) + (node.deepjump?node.deepjump->size():0) + (node.fakejump?node.fakejump->size():0)) * sizeof(int);
    }
    return index_size;
}
