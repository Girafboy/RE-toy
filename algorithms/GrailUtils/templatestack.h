...
// ********************************************************
    ...
    public:
        ...
        ~TemplateStack() { // if arrayData hasn't been freed we must free it
            if (arrayData != NULL)
                free(arrayData);
        }
        ...
        inline void Clear(void (*DestFunc)(Type)) { // calls DestFunc on everything in the array and then clears it
            for (int k = 0; k < currentSize; k++)
                DestFunc(arrayData[k]);
            currentSize = 0;
        }
