...
*******************************************************************************************/
    ...
    Grail::Grail(GrailGraph &graph, int Dim, int labelingType, bool pool, int poolsize) : g(graph), dim(Dim),
                                                                                          POOL(pool),
                                                                                          POOLSIZE(poolsize) {
        int i, maxid = g.num_vertices();
        visited = new int[maxid];
        QueryCnt = 0;
//	cout << "GrailGraph Size = " << maxid << endl;
        if (labelingType >= 2) {
...
*/
    ...
    void Grail::setIndex(GrailGraph &g, int traversal) {
        if (traversal == 0) {
...
//            random_shuffle(_index.begin(), _index.end());
    ...
    void Grail::setCustomIndex(GrailGraph &g, int traversal, int type) {
        int cnt = g.num_vertices();
        if (traversal == 0) {
            for (int i = 0; i < cnt; i++) {
            ...
            for (int i = 0; i < cnt; i++) {
                switch (type) {
                    case 2:
                        ...
                        break;
                    case 3:
                        customIndex[i] = min(customIndex[i],
                                             (double) g[i].post->at(traversal - 1) - g[i].pre->at(traversal - 1));
                        break;
                    case 4:
                    ...
                    case 5:
                        customIndex[i] = min(customIndex[i],
                                             g[i].post->at(traversal - 1) - g[i].pre->at(traversal - 1) - g.tcs(i));
                        break;
                }
            }
        }
...
// compute interval label for each node of tree (pre_order, post_order)
    void Grail::fixedreverselabeling(GrailGraph &tree, int traversal) {
...
// compute interval label for each node of tree (pre_order, post_order)
    void Grail::customlabeling(GrailGraph &graph, int traversal) {
...
// traverse tree to label node with pre and post order by giving a start node
    int Grail::customvisit(GrailGraph &tree, int vid, int &pre_post, vector<bool> &visited, int traversal) {
        visited[vid] = true;
...
// compute interval label for each node of tree (pre_order, post_order)
    void Grail::randomlabeling(GrailGraph &tree) {
        vector<int> roots = tree.getRoots();
        vector<int>::iterator sit;
        int pre_post = 0;
        vector<bool> visited(tree.num_vertices(), false);
...
// traverse tree to label node with pre and post order by giving a start node
    int Grail::visit(GrailGraph &tree, int vid, int &pre_post, vector<bool> &visited) {
...
// traverse tree to label node with pre and post order by giving a start node
    int Grail::fixedreversevisit(GrailGraph &tree, int vid, int &pre_post, vector<bool> &visited, int traversal) {
//	cout << "entering " << vid << endl;
        visited[vid] = true;
        GrailEdgeList el = tree.out_edges(vid);
...
*************************************************************************************/
    ...
    bool Grail::go_for_reach(int src, int trg) {
        ...
        GrailEdgeList el = g.out_edges(src);
        GrailEdgeList::iterator eit;
    ...
    bool Grail::go_for_reach_lf(int src, int trg) {
        ...
        if (g[src].top_level >=
            g[trg].top_level)        // if using level filter, reject if in a higher topological level
            return false;
