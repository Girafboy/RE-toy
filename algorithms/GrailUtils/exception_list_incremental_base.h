...
//	std::pair<VertexItr, VertexItr> vp;
        ...
        vector<int> counts;
        vector<double> times;
        static map<GrailVertex, pair<int, int>, VertexCompare> intervals;
        GrailVertexList vl;
        ...
        //bool operator<(const GrailVertex k1, const GrailVertex k2) { return k1.id < k2.id; };



        struct vertex_compare : public binary_function<GrailVertex, GrailVertex, bool> {
            bool operator()(const GrailVertex &x, const GrailVertex &y) {
                return intervals[x].second - intervals[x].first < intervals[y].second - intervals[y].first;
            }
        } vc;

        struct children_compare : public binary_function<GrailVertex, GrailVertex, bool> {
            bool operator()(const GrailVertex &x, const GrailVertex &y) {
        ...
        void getIntervals(GrailGraph &org, int index) {
...
//		}
        ...
        vector<pair<int, int> > findOpenIntervals(GrailGraph &g, const GrailVertex &v, vector<int> &extra) {
            vector<pair<int, int> > result;
            vector<GrailVertex> vlist;
            vector<GrailVertex>::iterator vit;
...
//			}
        ...
        void obtainDirectExceptions(GrailGraph &g) {
...
//						if(t1==out.second){
                            if (t1 == el.end()) {
                                if (!LEVEL_FILTER || vlist[i].min_parent_level <=
                                                     (*results)[k]->_v.top_level)        // it is "<=" instead of "<" to be able to capture indirects
                                    directs[vlist[i].id].insert((*results)[k]->_v.id);
                            }
                        }
                    }
        ...
        void obtainIndirectExceptions(GrailGraph &g) {
...
//					for(t2 = out.first; t2 != out.second ; t2++){
    ...
    public :
        ...
        ExceptionListIncrementalBase(GrailGraph &org) {
...
*/
    private:
        ...
        void obtainIncrementalExceptions(GrailGraph &g, int cur) {
            int i, j, k, count = 0, count2 = 0;
...
//						for(t1 = out.first; t1 != out.second ; t1++){
    ...
    map<GrailVertex, pair<int, int>, VertexCompare> ExceptionListIncrementalBase::intervals;
}
#endif
