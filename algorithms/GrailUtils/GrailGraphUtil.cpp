...
namespace grail {
    void
    GrailGraphUtil::dfs(GrailGraph &g, int vid, vector<int> &preorder, vector<int> &postorder, vector<bool> &visited) {
        visited[vid] = true;
        preorder.push_back(vid);
        GrailEdgeList el = g.out_edges(vid);
    ...
    void GrailGraphUtil::topo_leveler(GrailGraph &g) {
    ...
    int GrailGraphUtil::topo_level(GrailGraph &g, int vid) {
        ...
        int max = -1;
        g[vid].top_level = 0;
        GrailEdgeList el = g.in_edges(vid);
        GrailEdgeList::iterator eit;
        for (eit = el.begin(); eit != el.end(); eit++) {
...
// return reverse order of topological_sorting list
    void GrailGraphUtil::topological_sort(GrailGraph g, vector<int> &ts) {
        ...
        // for test
/*
	cout << "Topo sorting(GrailGraphUtil): ";
...
// implement tarjan's algorithm to find Strongly Connected Component from a given start node
    void GrailGraphUtil::tarjan(GrailGraph &g, int vid, int &index, hash_map<int, pair<int, int> > &order,
...
// return vertex map between old vertex and corresponding new merged vertex
    void GrailGraphUtil::mergeSCC(GrailGraph &g, int *on, vector<int> &reverse_topo_sort) {
        vector<int> sn;
...
// using GRIPP's labeling method
    void GrailGraphUtil::traverse(GrailGraph &tree, int vid, int &pre_post, vector<bool> &visited) {
        visited[vid] = true;
...
// compute interval label for each node of tree (pre_order, post_order)
    void GrailGraphUtil::pre_post_labeling(GrailGraph &tree) {
...
// for test
    void GrailGraphUtil::genRandomGraph(int n, double c, char *filename) {
