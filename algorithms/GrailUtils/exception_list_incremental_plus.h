...
namespace grail {
    ...
    public :
        ...
        ExceptionListIncrementalPlus(GrailGraph &org, int dim, bool lf) {
            int i;
            DIM = dim;
            LEVEL_FILTER = lf;
...
/*	
	void obtainInitialExceptions(const GrailGraph& g, GraphIndex& gi){
		...
		//			if(gi.contains(vlist[i],(*results)[k]->_v))
			...
			for(t1=out.first ; t1!=out.second; t1++){
				it.Insert(new VertexInterval((GrailVertex&)t1));
			}
			int t;
