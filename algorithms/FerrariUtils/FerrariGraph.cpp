 ...
 *
 * @file FerrariGraph.cpp
 * @author Stephan Seufert
 * @date 2012/10/31
 */
//--------------------------------------------------------------------------------------------------
#include "FerrariGraph.h"
#include <assert.h>
#include <fstream>
#include <iostream>
#include <sstream>
...
//--------------------------------------------------------------------------------------------------
namespace ferrari {
    FerrariGraph::FerrariGraph(const Graph &graph) : n(0), m(0) {
        n = graph.size();
        ...
        for (unsigned s = 0; s < n; ++s) {
            m += graph.getOutDegree(s);
            deg[s] = graph.getOutDegree(s);
            indeg[s] = graph.getInDegree(s);
            auto successors = graph.getSuccessors(s);
            nb[s] = {successors.begin(), successors.end()};
            auto predecessors = graph.getPredecessors(s);
            pd[s] = {predecessors.begin(), predecessors.end()};
            if (indeg[s] > 0) {
                is_root[s] = false;
            }
        }
 ...
 */
//--------------------------------------------------------------------------------------------------
    FerrariGraph::FerrariGraph(const std::string& filename) :
            n(0), m(0) {
        std::string line;
 ...
 */
//--------------------------------------------------------------------------------------------------
    const std::vector<unsigned>* FerrariGraph::get_neighbors(unsigned v) const {
        return &nb[v];
    }
//--------------------------------------------------------------------------------------------------
/**
 ...
 */
//--------------------------------------------------------------------------------------------------
    std::vector<unsigned>* FerrariGraph::get_roots() {
        return &roots;
    }
//--------------------------------------------------------------------------------------------------
