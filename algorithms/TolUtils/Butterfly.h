...
//            printf("Reconstruct Times: %d\n", levelhash.R);
        ...
        void convert_graph(const Graph &g) {
            N = g.size();
            ...
            for ( dir = 0 ; dir < 2 ; ++dir )
            {
                labels[dir].re_allocate(N);
                labels[dir].m_num = N;
                cost[dir].re_allocate(N);
                cost[dir].m_num = N;
                links[dir].re_allocate(N);
                links[dir].m_num = N;
            }
            q.re_allocate(N);
            level.re_allocate(N); level.m_num = N;
            l2n.re_allocate(N); l2n.m_num = N;

            sum = new int[N+1];
            memset(sum,0,(N+1)*sizeof(int));

            for (int x = 0; x < N; ++x) {
                for (const auto &y : g.getSuccessors(x)) {
                    links[0][x].push_back(y);
                    links[1][y].push_back(x);
                }
            }
...
//                                if ( AddLabel( c, p, 0, id ) && c != p ) lcnt++;
        ...
        void output( string st )
        {
            ...
            string s1 = "../" + st + ".label";
            string s2 = "../" + st + ".index";
            FILE *file = fopen(s1.c_str(),"w");
            FILE *ifile = fopen(s2.c_str(),"w");
        ...
        // 自己加的
        pair<vector<int>, vector<unsigned int>> generate_index_storage() {
            // file->label, ifile->index
            vector<int> local_labels;
            vector<unsigned int> local_index;
            compute_order();

            int x = 0;
            for ( int i = 0 ; i < N ; ++i )
            {
                local_index.push_back(x);

                local_labels.push_back(order[0][i]);
                x++;

                for ( int j = 0 ; j < labels[0][i].m_num ; ++j )
                {
                    local_labels.push_back(labels[0][i][j]);
                    x++;
                }

                local_labels.push_back(-1);
                x++;
                ...
                for ( int j = 0 ; j < labels[1][i].m_num ; ++j )
                {
                    local_labels.push_back(labels[1][i][j]);
                    x++;
                }

                local_labels.push_back(-2);
                x++;
            }
            return make_pair(local_labels, local_index);
        }
