...
//    free(dag);
    ...
    void topoSort_B(int *&topo) {  //original graph's topological level
        int head = 0, tail = 0;
        int *Q_B = (int *) malloc(sizeof(int) * (vertexN));
        topo = (int *) malloc(sizeof(int) * (vertexN));
...
//    free(topo);
            ...
            if (L[Q[head]] == l) {   //from the first level, namely from Q[0]
                ...
                for (int j = start[s]; j < (start[s] + outd[s]); ++j) {
                    ...
                    if (L[t] == (l + 1))
                        ...
                        if (((L[t] - l) > 2) & (L[t] % 2 == 1) &
                            (l % 2 == 1)) //both vertex are in even level and the difference is more than 2
                            ficout[s] = ficin[t] = 1;
                        else {
                            if (((L[t] - l) == 2) &
                                (L[t] % 2 == 1)) //both vertex are in even level and the difference equals to 2
                                ficout[s] = 1;
                            else {
                                if ((l % 2 == 0) & (L[t] % 2 ==
                                                    1)) //the start vertex is in odd level while the end vertex is in even level
                                    ficin[t] = 1;
                                else {
                                    if ((l % 2 == 1) & (L[t] % 2 ==
                                                        0)) //the start vertex is in even level while the end vertex is in odd level
                                        ficout[s] = 1;
                                }
                            }
                        }
    ...
    int interconnect(int numb, FILE *pf,
                     int nround) //numb is the length of queue(store the remaining vertex in ascending of level )
    {
                    ...
                    if (i > j) {    //when it is in odd level
                        ...
                        while ((L[Q[j]] % 2 == 1) & (j < numb)) {
                            if (ficin[Q[j]] == 1) {
                                ...
                                indexG[Q[j] % vertexN + vertexN] = cur, cur++, Lnew[Q[j] % vertexN + vertexN] =
                                        L[Q[j]] / 2;
                            }
                            j++;
                        }
            ...
            if (((ind[curV] != 0) || (outd[curV] != 0))) {
                if ((L[curV] % 2 == 0)) {  //if start vertex is in odd level
                    for (int e = start[curV]; e < (start[curV] + outd[curV]); ++e) {
                        ...
                        if ((L[curE] % 2 == 1) &&
                            (L[curE] != (L[curV] + 1))) //the edge is in even level but not neighbor
                    ...
                    for (int rev = startR[curV]; rev < (startR[curV] + ind[curV]); ++rev) {
                        ...
                        if ((L[curE] == L[curV] - 1) |
                            (L[curE] == L[curV] - 2)) //if the end vertex is in neighbor level then store in "reverse"
                        {
                            reverse.push_back(curE);
                            SLin[nround].push_back(
                                    curE), countin++; //SL in label record its in neighbors and 2(considering 2-4)
                        }
                    }
                    ...
                    for (int forw = start[curV]; forw < (start[curV] + outd[curV]); ++forw) {
                        ...
                        if ((L[curE] == L[curV] + 1)) {
                            ...
                            if (((L[curE] - L[curV]) > 2) &
                                (L[curE] % 2 == 1)) //both in even level and difference is more than 2
                                newG[cVnew].push_back(
                                        curE % vertexN + vertexN); // then store it as an edge of vertex Q[j]
...
//            printf("after %d's round, %d vertex,%d edge,%d LEVEL \n", nr, QnumNew, edgeN, level);
    ...
    void labelConstruction(vector<int> *&SL) {
...
//            printf("%d round begin", r + 1);
            ...
            for (int i = 0; i < rN[r]; i++) {
                ...
                if ((curVr < vertexN) & (R[curVr % vertexN] == r)) {
                    ...
                    for (int j = startSLout1[r][i]; j < startSLout1[r][i + 1]; j++) {  //original out label
                        ...
                        //three cases
                        if (SLout1[r][j] !=
                            curVr) { //if its fictitious out then we have already do this in last step???
                            ...
                            if ((curEr < vertexN) & (R[curEr % vertexN] > r)) { //if it is original vertex
                                ...
                                if (curEr < vertexN) { //fictitious out
                                    for (int k = TLout_start[curEr];
                                         k < TLout_start[curEr + 1]; k++) { // the latest curEr'S ls LABEL
                                        SL[curVr].push_back(TLout[k]);
                                    }
                                } else {  //fictious in vertex
                                    for (int k = TLin_start[curEr - vertexN];
                                         k < TLin_start[curEr - vertexN + 1]; k++) {
                        ...
                        if (curEr != (curVr + vertexN)) {
                            ...
                            if ((curEr < vertexN) & (R[curEr % vertexN] > r)) { //if it is original vertex
                                for (unsigned int k = 0;
                                     k < SL[curEr + vertexN].size(); k++) { // the latest curEr'S ls LABEL
                                    SL[curVr + vertexN].push_back(SL[curEr + vertexN][k]);
                                }
                            } else {
                                if (curEr < vertexN) { //fictitious out
                                    for (int k = TLout_start[curEr + vertexN];
                                         k < TLout_start[curEr + vertexN + 1]; k++) { // the latest curEr'S ls LABEL
            ...
            deleV1[r] = nullptr;
            free(startSLout1[r]);
            startSLout1[r] = nullptr;
            free(startSLin1[r]);
            startSLin1[r] = nullptr;
            free(TLout);
            TLout = nullptr;
            free(TLin);
            TLin = nullptr;
...
//    fclose(bfile3);
    ...
    void transform() {
        vector<int> a1[rround], a2[rround], a3[rround], a4[rround], a5[rround];
        for (int i = 0; i < rround; i++) {
            rN[i] = deleV[i].size();
        }
        SLin1 = (int **) malloc(rround * sizeof(int *));
        for (int i = 0; i < rround; i++) {
        ...
        SLout1 = (int **) malloc(rround * sizeof(int *));
        for (int i = 0; i < rround; i++) {
        ...
        deleV1 = (int **) malloc(rround * sizeof(int *));
        for (int i = 0; i < rround; i++) {
        ...
        startSLout1 = (int **) malloc(rround * sizeof(int *));
        for (int i = 0; i < rround; i++) {
        ...
        startSLin1 = (int **) malloc(rround * sizeof(int *));
        for (int i = 0; i < rround; i++) {
