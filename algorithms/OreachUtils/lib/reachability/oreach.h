...
//template <unsigned int NumSupports>
struct NodeInfo {
#if NUM_SUPPORTS > 32
...
#endif
//    using SVType = typename std::conditional<(NumSupports > 32), uint64_t,
//            typename std::conditional<(NumSupports > 16), uint32_t,
//                    typename std::conditional<(NumSupports > 8), uint16_t, uint8_t>::type>::type>::type;
...
small_component:
        ...
        unsigned long long getIndexSize() const {
            return nodes.size() * (12 + 12 * NUM_TOPSORTS + NUM_SUPPORTS / 8 * 2) + condensation.number_of_edges() * sizeof(int);
        }
