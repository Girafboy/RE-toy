...
#include "algorithms/OReachWrapper.h"
...
int main(int argc, char *argv[]) {
    ...
    while (i < argc) {
        ...
        if (strcmp("--time", argv[i]) == 0) {  // timeout second
            ...
            } else if (algorithm_name == "oreach") {
                if (i > argc) {
                    algorithmUsage(algorithm_name);
                    return 0;
                }
                algorithm = new OReachWrapper();
            } else {
    ...
    if (!file_path.empty()) {  // read graph from file
        ...
        graph = new Graph(file_path, graph_name, false);
